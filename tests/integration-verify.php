...
function check( $n, $fn ) {
	...
	try {
		$r = $fn();
		if ( $r === true || is_string( $r ) ) {
			echo ( is_string( $r ) ? $r : 'PASS' ) . "\n";
			++$ok;
		} else {
			echo "FAIL\n";
			++$fail;}
...
clearstatcache();
$t0 = microtime( true ); for ( $i = 0;$i < 10;$i++ ) {
	file_exists( "$B/miss-$i.txt" ); }
...
check(
	'getimagesize on stream',
	function () use ( $B ) {
		$x = @getimagesize( "$B/i.png" );
		return $x ? "PASS {$x[0]}x{$x[1]}" : false;
	}
);
check(
	'copy to local temp (EXIF path)',
	function () use ( $B ) {
		$t  = tempnam( sys_get_temp_dir(), 'a' );
		$r  = copy( "$B/i.png", $t );
		$sz = filesize( $t );
		unlink( $t );
		return $r && $sz === 79 ? "PASS ($sz bytes)" : false;
	}
);
check(
	'unlink',
